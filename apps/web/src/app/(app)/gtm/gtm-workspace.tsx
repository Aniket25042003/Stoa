...
export function GtmWorkspace({
  ...
  const loadPlan = useCallback(async (companyId: string) => {
    ...
    try {
      ...
      setPlan(null);
      setMessages([]);
      setNotice("Could not load the strategy workspace.");
    } finally {
      if (requestId !== loadRequestRef.current) return;
      setLoading(false);
    }
  ...
  async function generatePlan() {
    ...
    try {
      ...
      setNotice("Your strategy blueprint has started. Open the status page to review and approve it.");
      setMessages((cur) => [...cur, { role: "system", content: "Strategy blueprint generation started." }]);
      if (body.run_id) {
        setMessages((cur) => [...cur, { role: "assistant", content: `Open the status page when you are ready: /runs/${body.run_id}` }]);
      }
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Could not start strategy blueprint");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function uploadPlan(event: React.FormEvent<HTMLFormElement>) {
    ...
    const title = String(form.get("title") ?? "Strategy blueprint").trim() || "Strategy blueprint";
    if (content.length < 20) {
      setNotice("Paste a strategy blueprint with at least 20 characters.");
      return;
    }
    setBusy(true);
    setNotice(null);
    try {
      ...
      const body = await res.json();
      if (!res.ok) throw new Error(body.detail || "Could not upload strategy blueprint");
      setPlan(body.plan);
      setMessages([{ role: "system", content: "Uploaded an existing strategy blueprint." }]);
      if (uploadRef.current) uploadRef.current.value = "";
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Could not upload strategy blueprint");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function sendMessage(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body = await res.json();
      if (!res.ok) throw new Error(body.detail || "Could not update strategy");
      setPlan(body.plan);
      setMessages((cur) => [...cur, body.assistant]);
    } catch (error) {
      setMessages((cur) => [...cur, { role: "assistant", content: error instanceof Error ? error.message : "Could not update strategy" }]);
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-[2rem] bg-slate-deep p-7 text-white shadow-card md:p-10">
        <p className="eyebrow text-primary">Strategy</p>
        <h1 className="mt-4 font-display text-4xl font-extrabold tracking-[-0.045em] md:text-5xl">
          {activeCompany?.name ?? "Brand"} Strategy workspace
        </h1>
        <p className="mt-4 max-w-2xl text-sm leading-7 text-white/68">
          Create a strategy, upload an existing one, then chat through changes while keeping context tied to the selected brand.
        </p>
      </section>
      ...
      {loading ? (
        <div className="rounded-3xl p-8 text-center card-glass">Loading strategy workspace...</div>
      ) : !plan ? (
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-3xl p-7 card-glass">
            <p className="eyebrow">Option one</p>
            <h2 className="mt-3 font-display text-3xl font-bold tracking-[-0.03em] text-on-surface">Create your strategy</h2>
            <p className="mt-3 text-sm leading-7 text-on-surface-variant">
              Use your brand profile to create a first plan for review. You can approve, revise, and keep improving it from here.
            </p>
            <button type="button" disabled={busy} onClick={() => void generatePlan()} className="btn-primary mt-6 px-5 py-3 text-sm disabled:opacity-60">
              {busy ? "Starting..." : "Generate Strategy"}
            </button>
          </div>

          <form onSubmit={(event) => void uploadPlan(event)} className="rounded-3xl p-7 card-glass">
            <p className="eyebrow">Option two</p>
            <h2 className="mt-3 font-display text-3xl font-bold tracking-[-0.03em] text-on-surface">Bring your own</h2>
            <label className="mt-5 grid gap-2 text-sm font-semibold text-on-surface">
              Strategy title
              <input name="title" defaultValue={`${activeCompany?.name ?? "Brand"} strategy`} className="rounded-2xl border border-outline-variant/70 bg-surface-container-low px-4 py-3 font-normal outline-none focus:border-primary" />
            </label>
            <label className="mt-4 grid gap-2 text-sm font-semibold text-on-surface">
              Strategy content
              <textarea ref={uploadRef} name="content_markdown" rows={10} className="rounded-2xl border border-outline-variant/70 bg-surface-container-low px-4 py-3 font-normal outline-none focus:border-primary" placeholder="Paste your existing strategy..." />
            </label>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-3xl p-6 card-glass md:p-7">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="eyebrow">Active Strategy</p>
                <h2 className="mt-2 font-display text-3xl font-bold tracking-[-0.03em] text-on-surface">{plan.title}</h2>
                <p className="mt-2 text-sm text-on-surface-variant">Source: {plan.source === "uploaded" ? "Your upload" : "Created for you"}</p>
              </div>
          ...
          <div className="rounded-3xl p-6 card-glass md:p-7">
            <p className="eyebrow">Refine your strategy</p>
            <div className="mt-5 max-h-[520px] space-y-3 overflow-auto pr-1">
              ...
              {messages.filter((message) => message.role !== "system").length === 0 ? (
                <p className="rounded-2xl bg-surface-container-low px-4 py-3 text-sm text-on-surface-variant">
                  Ask for strategy updates, positioning refinements, or channel ideas.
                </p>
              ) : null}
            </div>
            <form onSubmit={(event) => void sendMessage(event)} className="mt-5 flex gap-2">
              <input name="content" disabled={busy} className="min-w-0 flex-1 rounded-2xl border border-outline-variant/70 bg-surface-container-low px-4 py-3 text-sm outline-none focus:border-primary" placeholder="Ask Stoa to update your strategy..." />
