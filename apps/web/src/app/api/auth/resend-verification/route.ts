...
export async function POST(request: Request) {
  ...
  const rateLimited = await enforceAuthRateLimit(request, email, "auth_resend");
  if (rateLimited) return rateLimited;

  return proxyJsonResponse(request, "/v1/auth/resend-verification", { method: "POST", body: bodyText });
}
